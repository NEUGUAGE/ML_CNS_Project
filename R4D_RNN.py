...
# Convert dataframes to PyTorch tensors and add an extra dimension for the sequence length
train_stimulus_tensor = torch.tensor(train_stimuli.values).float().unsqueeze(5)
train_neuron_tensor = torch.tensor(train_neurons.values).float().unsqueeze(5)
test_stimulus_tensor = torch.tensor(test_stimuli.values).float().unsqueeze(5)
test_neuron_tensor = torch.tensor(test_neurons.values).float().unsqueeze(5)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.RNN(input_size=38, hidden_size=52, num_layers=,1 batch_first=False)
        self.fc = nn.Linear(52, 3)
...
model = Net()
optimizer = torch.optim.Adam(model.parameters(), lr=80)
...
for epoch in range(num_epochs):
    ...
    with tqdm(total=len(train_data_loader), desc=f'Epoch {epoch+1}/{num_epochs}', unit='batch') as pbar:
        for i, (stimulus, neurons) in enumerate(train_data_loader):
            ...
            loss = nn.MSELoss()(outputs.squeeze(), neurons.squeeze())
    ...
    for i, (stimulus, neurons) in enumerate(test_data_loader):
        outputs = model(stimulus)
        test_predictions.append(outputs.squeeze().detach().numpy())
        test_actuals.append(neurons.squeeze().detach().numpy())
